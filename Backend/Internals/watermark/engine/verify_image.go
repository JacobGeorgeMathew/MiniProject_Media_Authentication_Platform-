...
func Identify(img image.Image, c []Constants) (x int, y int, flag bool) {
	...
	for i := 0; i+256 <= h; i++ {
		...
		go func(i int) {
			...
			strip := make([][]float64, 256)
			for row := 0; row < 256; row++ {
				strip[row] = make([]float64, 16)
				copy(strip[row], Ymatrix[i+row][:16])
			}

			if Verifytile(strip, c, false) {
				mu1.Lock()
				if x_index == -1 || i < x_index {
					x_index = i
				}
				mu1.Unlock()
			}
	...
	for j := 0; j+256 <= w; j++ {
		...
		go func(j int) {
			...
			strip := make([][]float64, 16)
			for row := 0; row < 16; row++ {
				strip[row] = make([]float64, 256)
				copy(strip[row], Ymatrix[x_index+row][j:j+256])
			}

			if Verifytile(strip, c, true) {
				mu2.Lock()
				if y_index == -1 || j < y_index {
					y_index = j
				}
				mu2.Unlock()
			}
