    ...
    <div className="min-h-screen bg-slate-950 flex items-center justify-center px-4"
      ...
      <div className="relative z-10 w-full max-w-sm">
        <div className="text-center mb-8">
          ...
          <h1 className="text-xl font-black text-slate-100">Enterprise Sign in</h1>
          <p className="text-xs text-slate-500 mt-1">Access your API platform dashboard</p>
        </div>

        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="block text-xs text-slate-500 mb-1.5 tracking-wide">Company email</label>
            <input
              type="email"
              required
              value={form.email}
              onChange={(e) => setForm({ ...form, email: e.target.value })}
              className="w-full bg-slate-900 border border-slate-700 rounded-lg px-4 py-2.5 text-sm text-slate-100 placeholder-slate-600 focus:outline-none focus:border-violet-500 transition-colors"
        ...
        <p className="text-center text-xs text-slate-600 mt-6">
          ...
          <Link to="/enterprise/register" className="text-violet-400 hover:text-violet-300 transition-colors">
            Register
          </Link>
        </p>
